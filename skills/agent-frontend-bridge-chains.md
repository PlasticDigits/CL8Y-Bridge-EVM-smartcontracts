...
Use when wiring **new EVM / Cosmos / Solana bridge peers into the SPA**, debugging **Registered Chains** or **`getChainsForTransfer`**, or explaining why **operator-only `MEGAETH_*`** (no `VITE_`) never reaches **`import.meta.env`**. For **Solana wallet txs / blockhash / GL-128**, see [`agent-solana-tx-blockhash.md`](./agent-solana-tx-blockhash.md). For **Terra LCD rate-limit vs EVM parity (decimal-normalized payout only; GL-130)**, see [`FRONTEND_BRIDGE_INVARIANTS.md`](../docs/FRONTEND_BRIDGE_INVARIANTS.md) **INV-UX2-TERRA1** — `queryTerraRateLimitStatus`. For **token PNG / `LOGO_SYMBOLS` (GL-133)**, see [`agent-frontend-token-logos.md`](./agent-frontend-token-logos.md) (**INV-FE-TOKEN-LOGO-1**). For **Transfer token picker economic-then-test order (GL-136)**, see [`agent-frontend-token-rank.md`](./agent-frontend-token-rank.md) (**INV-FE-TOKEN-RANK-1**). For **Legal clickwrap on mutative CTAs (GL-134)**, see [`agent-frontend-clickwrap.md`](./agent-frontend-clickwrap.md) (**INV-FE-CLICKWRAP-1**). For **Android Chrome Terra connect / WalletConnect Open+Copy (GL-137)**, see [`agent-frontend-terra-wallet-mobile.md`](./agent-frontend-terra-wallet-mobile.md) (**INV-FE-WC-MOBILE-1**). For **Terra hash list vs active-withdrawal index (GL-139)**, see [`agent-terraclassic-active-withdrawals.md`](./agent-terraclassic-active-withdrawals.md) (**INV-FE-TC-AW1**).
...
| EVM deposit `allowance` / `balanceOf` / receipt wait on source RPC (not wallet-bound wagmi / thirdweb) | [`useBridgeDeposit`](../packages/frontend/src/hooks/useBridgeDeposit.ts), [`getEvmClient`](../packages/frontend/src/services/evmClient.ts), [`FRONTEND_BRIDGE_INVARIANTS.md`](../docs/FRONTEND_BRIDGE_INVARIANTS.md) **INV-FE-EVM-ALLOWANCE-1** (GL-133 follow-up) |
...
- **INV-FE-EVM-ALLOWANCE-1 (GL-133):** EVM deposit **`allowance`/`balanceOf`/receipt waits** use **`getEvmClient(sourceChainConfig)`**, not wagmi’s wallet-bound client — avoids false “token may not exist” and false “Deposit transaction timed out” / missing **xchainHashId** when the wallet RPC 429s (e.g. thirdweb) but the tx already mined.
- **INV-FE-CLICKWRAP-1 (GL-134):** Mutative deposit/submit/execute is gated on Legal **`signed_latest`** for property **`bridge.cl8y.com`**. See [`agent-frontend-clickwrap.md`](./agent-frontend-clickwrap.md).
