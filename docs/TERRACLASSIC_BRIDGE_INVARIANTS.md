...
| **Same code_id continue** | If columbus-5 rejects wasm migrate to the same `code_id`, admin calls `ContinueActiveIndexMigrate { limit, rebuild: false }` until complete. |
...
| **Pagination** | Prefer `next_start_after` from `active_withdrawals`. A short or empty page with that cursor set is not exhausted (skip cap). |
