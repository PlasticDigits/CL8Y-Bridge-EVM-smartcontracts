...
| EVM deposit `allowance` / `balanceOf` on source RPC (not wallet-bound wagmi) | [`useBridgeDeposit`](../packages/frontend/src/hooks/useBridgeDeposit.ts), [`getEvmClient`](../packages/frontend/src/services/evmClient.ts), [`FRONTEND_BRIDGE_INVARIANTS.md`](../docs/FRONTEND_BRIDGE_INVARIANTS.md) **INV-FE-EVM-ALLOWANCE-1** (GL-133 follow-up) |
...
- **INV-FE-EVM-ALLOWANCE-1 (GL-133):** EVM deposit **`allowance`/`balanceOf`** use **`getEvmClient(sourceChainConfig)`**, not wagmi’s wallet-bound client — avoids false “token may not exist” when balance already renders (e.g. BSC vFDUSD → Terra).
